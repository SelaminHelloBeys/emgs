...
import { Users, Video, Megaphone, Plus, ChevronRight, Loader2 } from 'lucide-react';

interface SchoolStats { totalLessons: number; totalAnnouncements: number; }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setStats({ totalLessons: lessonsRes.count || 0, totalAnnouncements: announcementsRes.count || 0 });
          ...
          {announcements.length === 0 ? (
            ...
            <Stagger className="space-y-4">
              {announcements.map((ann) => (
                <MotionItem key={ann.id} variant="slideRight">
